...
class SkullApp : public D3D11App
{
public:
...
: D3D11App(hInstance), mVB(0), mIB(0), mFX(0), mTech(0),
...
{
	m_mainWndCaption = L"07_D3DLoadShapesFromModels";
...
void SkullApp::DrawScene()
{
    ...
    UINT offset = 0;
    m_pD3DImmediateContext->IASetVertexBuffers(0, 1, &mVB, &stride, &offset);
	m_pD3DImmediateContext->IASetIndexBuffer(mIB, DXGI_FORMAT_R32_UINT, 0);
    ...
    for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		m_pD3DImmediateContext->DrawIndexed(mSkullIndexCount, 0, 0);
    }
...
void SkullApp::BuildGeometryBuffers()
{
	...
	for(UINT i = 0; i < vcount; ++i)
	{
		...
		vertices[i].Color = reinterpret_cast<const float*>(&Colors:: Black);
